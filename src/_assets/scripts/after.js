...
document.querySelectorAll('.shiki-wrapper').forEach((block) => {
	const codeToolbar = block.querySelector('.code-toolbar');
	codeToolbar.classList.remove('hidden');
	codeToolbar.removeAttribute('aria-hidden');

	const copyButton = codeToolbar.querySelector('.copy-button');
	...
	const linkButton = codeToolbar.querySelector('.link-button');
	linkButton.innerHTML = ICON_LINK;
	linkButton.addEventListener('click', () => {
		const codeUrl =
			window.location.href.split('#')[0] + '#' + block.getAttribute('id');
